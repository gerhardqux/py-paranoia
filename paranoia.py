...
#
# 1.4.0 - 15 Mar 2020
# + Remove save game functionality
# + Remove go-to-page cheat
# + Port to python3
# + Conform to PEP8 codestyle
#
# 1.3.2 - 11 Oct 2013
...
from builtins import chr, input
...
""")
    ...
    def choose(self, choices):
        ...
        letter_page_map = dict()
        letters = list()
        choice_prompt = ""
        for i, (page, choice) in enumerate(choices.items()):
            letter = chr(i + 97)
            letters.append(letter)
            choice_prompt += " {} - {}\n".format(letter, choice)
            letter_page_map[letter] = page
        choice_prompt += "\nSelect %s or %s:\n" % (
            ", ".join(c for c in letters[:-1]),
            letters[-1])
        choice = None
        while (choice not in letter_page_map and
            ...
            print(choice_prompt)
            try:
                choice = input("Type choice and press <Enter> to continue: ")
            except EOFError:
                print()
                choice = 'q'
            if choice == 'p':
...
Equipment: Red Reflec Armour, Laser Pistol, Laser Barrel (red),
          ...
          Utility Belt & Pouches""" % self.clone)
        print("=" * 79)
        print()
...
""" % self.plato_clone)
    ...
    def page40(self):
        print("""\
You breathe a sigh of relief as Plato-B-PHI%d picks on the other
Troubleshooter. "You down here in the front," says the instructor pointing at
the other Troubleshooter, "you'll make a good volunteer.  Please step forward."
...
""")
    ...
    def quit(self):
        ...
        while choice not in ['y', 'n']:
            try:
                choice = input("Quit this game? [y/n] ")
            except EOFError:
                print()
            if choice == 'y':
                sys.exit(0)
